...
static bool compare(const struct list_elem *one, const struct list_elem *two, void *aux UNUSED);
...
compare(const struct list_elem *one, const struct list_elem *two, void *aux UNUSED)
{
    ...
    if (tOne->wakeup_ticks < tTwo->wakeup_ticks)
    {
      	return true;
    }
	else if (tOne->wakeup_ticks == tTwo->wakeup_ticks) {
...
/* Called from timer_interupt
 * For each thread created, call sema_up to thread_unblock
 */
static void
